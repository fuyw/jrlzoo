    ...
    def update(self, replay_buffer, model_buffer):
        if self.update_step % 1000 == 0:
            ...
            for i in range(self.rollout_batch_size):
                ...
                env_state.qpos[:] = batch.qpos[i]
                env_state.qvel[:] = batch.qvel[i]
...
task_name = 'Hopper'
env = gym.make(f'{task_name.lower()}-medium-v2')
obs_dim = env.observation_space.shape[0]
act_dim = env.action_space.shape[0]
...
replay_buffer.load(f'saved_buffers/{task_name}-v2/s0.npz')

agent = OracleAgent(env=f'{task_name.lower()}-medium-v2',
