"""
    import numpy as np
    buffer = ReplayBuffer(observation_shaep=(84, 84),
                          stack_size=4,
                          replay_capacity=1000,
                          batch_size=32)

    for _ in range(50):
        obs = np.random.randint(0, 2, size=(84, 84))
        buffer.add(observation=obs,
                   action=0,
                   reward=0,
                   terminal=0)

    batch = buffer.sample_transition_batch(batch_size=32)

    np.savez("saved_buffer",
             observations=buffer.observation,
...
class ReplayBuffer:
    ...
    """
        ...
        self.add_count += 1
        self.invalid_range = invalid_range(self.cursor(),
    ...
    """
        ...
        while (len(indices) < batch_size
               and attempt_count < self._max_sample_attempts):
        ...
        for batch_element, state_index in enumerate(indices):
            ...
            if not is_terminal_transition:
                ...
                trajectory_length = np.argmax(
                    trajectory_terminals.astype(bool), 0) + 1
        ...
        for member_name, member in self.__dict__.items():
            if member_name == '_store':
                for array_name, array in self._store.items():
                    checkpointable_elements[STORE_FILENAME_PREFIX +
                                            array_name] = array
