...
def get_args():
    ...
    parser.add_argument("--start_timesteps", type=int, default=2_000)
...
def train_and_evaluate(args):
    t1 = time.time()
    exp_name = f"finetune_{args.agent}_{args.env_name[:-3].lower()}"
    ...
    raw_reward = eval_policy(agent, eval_env, args.seed)
    agent.load("saved_models/dqn_mountaincar_3")
    trained_reward = eval_policy(agent, eval_env, args.seed)
    print(f"raw_reward = {raw_reward:.2f} ==> trained_reward = {trained_reward:.2f}")
