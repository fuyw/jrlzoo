...
def plot_tmp(env_name="hopper-medium-v2"):
    _, axes = plt.subplots(nrows=2, ncols=3, figsize=(16, 8))
    df = pd.read_csv(f"tmp_logs/{env_name}/iql_s0.csv", index_col=0).dropna()
    df.index = df['step'].apply(lambda x: int(x))
    plt_idx = range(10000, 1010000, 10000)
    cols = ['reward', 'critic_loss', 'value_loss', 'actor_loss', 'q1', 'v']
    for i, col in enumerate(cols):
        ax = axes[i // 3][i % 3]
        ax.plot(plt_idx, df.loc[plt_idx, col].values, label=col)
        ax.set_title(f"{col} on {env_name}")
    plt.tight_layout()
    plt.savefig(f'imgs/origin_{env_name}.png', dpi=360)
