...
class COMBOAgent:
    def __init__(self,
        ...
        print(f'Load static_fn: {self.model.static_fn}')
    ...
    def train_step(self,
        ...
        def loss_fn(actor_params: FrozenDict,
            ...
            # cql1_loss = self.min_q_weight * (ood_q1 - q1)
            # cql2_loss = self.min_q_weight * (ood_q2 - q2)

            cql1_loss = self.min_q_weight * (ood_q1 - q1 * mask * self.real_loss_ratio)
            cql2_loss = self.min_q_weight * (ood_q2 - q2 * mask * self.real_loss_ratio)
            ...
            # total_loss = critic_loss * mask * self.real_loss_ratio + \
            #     actor_loss * mask * self.real_loss_ratio + \
            #     alpha_loss * mask * self.real_loss_ratio + \
            #     (cql1_loss + cql2_loss) * mask * self.real_loss_ratio
    ...
    def update(self, replay_buffer, model_buffer):
        ...
        log_info['real_batch_rewards'] = abs(real_batch.rewards).mean()
        log_info['real_batch_actions'] = abs(real_batch.actions).sum(1).mean()
        log_info['real_batch_dones'] = abs(1 - real_batch.discounts).mean()
        log_info['model_batch_obs'] = abs(model_batch.observations).sum(1).mean()
        log_info['model_batch_rewards'] = abs(model_batch.rewards).mean()
        log_info['model_batch_actions'] = abs(model_batch.actions).sum(1).mean()
        log_info['model_batch_dones'] = abs(1 - model_batch.discounts).mean()
