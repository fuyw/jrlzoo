...
from tf_utils import get_required_argument, TensorStandardScaler
from fc import FC

from utils2.logging import Progress, Silent
...
class BNN:
    def __init__(self, params):
        ...
        self.reward_loss, self.next_state_loss = None, None
        ...
        self.reward_classification = params.get('reward_classification', False)
        ...
        self.gradient_penalty = params.get('gradient_penalty', 0.0)
        self.gradient_penalty_scale = params.get('gradient_penalty_scale',
                                                 10.0)
        self.separate_mean_var = params.get('separate_mean_var', False)
    ...
    def add(self, layer, var_layer=False):
        ...
        if not var_layer:
            if len(self.layers) == 0 and layer.get_input_dim() is None:
                raise ValueError(
                    "Must set input dimension for the first layer.")
        else:
        ...
        print('Added layer with input dim', layer.get_input_dim(),
              ', output dim', layer.get_output_dim())
    ...
    def finalize(self, optimizer, optimizer_args=None, *args, **kwargs):
        ...
        Returns: None
        """
        if len(self.layers) == 0 or (self.separate_mean_var
                                     and len(self.var_layers) == 0):
        ...
        if not self.separate_mean_var:  # True
            self.layers[-1].set_output_dim(2 *
                                           self.layers[-1].get_output_dim())
            ...
            with self.sess.as_default():
                with tf.variable_scope(self.name):
                    ...
                    self.max_logvar = tf.Variable(
                        np.ones([1, self.layers[-1].get_output_dim() // 2]) /
                        2.,
                        dtype=tf.float32,
                        name="max_log_var")
            ...
            with self.sess.as_default():
                with tf.variable_scope(self.name):
                    self.scaler = TensorStandardScaler(
                        self.layers[0].get_input_dim())
                    self.max_logvar = tf.Variable(
                        np.ones([1, self.var_layers[-1].get_output_dim()]) /
                        2.,
                        dtype=tf.float32,
                        name="max_log_var")
                    self.min_logvar = tf.Variable(
        ...
        with tf.variable_scope(self.name):
            ...
            if self.multi_step_prediction:
                self.sy_train_in = tf.placeholder(
                    dtype=tf.float32,
                    shape=[
                        self.num_nets, None, self.num_plan_steps,
                        self.layers[0].get_input_dim()
                    ],
                ...
                self.sy_train_in = tf.placeholder(
                    dtype=tf.float32,
                    shape=[
                        self.num_nets, None, self.layers[0].get_input_dim()
                    ],
                    name="training_inputs")
                if not self.separate_mean_var:  # True
                    self.sy_train_targ = tf.placeholder(
                    ...
                    self.sy_train_targ = tf.placeholder(
                        ...
                        name="training_targets")
            if not self.multi_step_prediction:  # True
                if not self.deterministic:  # True
                    ...
                    train_loss += 0.01 * tf.reduce_sum(
                        self.max_logvar) - 0.01 * tf.reduce_sum(
                            self.min_logvar)
                else:
                    train_loss = self._compile_losses(self.sy_train_in,
                                                      self.sy_train_targ,
                                                      inc_var_loss=False)
                    train_loss += tf.add_n(self.decays)
                self.mse_loss = self._compile_losses(self.sy_train_in,
                                                     self.sy_train_targ,
                                                     inc_var_loss=False)
                self.reward_loss, self.next_state_loss = self._compile_rew_dyn_losses(
                    self.sy_train_in, self.sy_train_targ)
                if self.reward_classification:
                    self.reward_acc = self._compile_rew_prediciton_accuracy(
                        self.sy_train_in, self.sy_train_targ)
                if self.gradient_penalty > 0.0:
                    mean_out, _ = self._compile_outputs(self.sy_train_in,
                                                        ret_log_var=True)
                    ddx = tf.gradients(mean_out, self.sy_train_in)[0]
                    ddx = tf.sqrt(tf.reduce_sum(tf.square(ddx), axis=-1))
                    ddx = tf.reduce_mean(
                        tf.square(ddx - self.gradient_penalty) *
                        self.gradient_penalty_scale)
                    train_loss = train_loss + ddx
            ...
            # False
                ...
                if not self.deterministic:  # True
                    train_loss = tf.reduce_sum(self._compile_losses(
                        self.sy_train_in,
                        self.sy_train_targ,
                        inc_var_loss=True),
                                               axis=0)
                    train_loss = [train_loss[i] + tf.add_n(self.decays) + \
                                0.01 * tf.reduce_sum(self.max_logvar) - 0.01 * tf.reduce_sum(self.min_logvar) for i in range(self.num_plan_steps)]
                else:
                    train_loss = tf.reduce_sum(self._compile_losses(
                        self.sy_train_in,
                        self.sy_train_targ,
                        inc_var_loss=False),
                                               axis=0)
                    train_loss = [
                        train_loss[i] + tf.add_n(self.decays)
                        for i in range(self.num_plan_steps)
                    ]
                self.mse_loss = [
                    tf.reduce_mean(self._compile_losses(
                        self.sy_train_in,
                        self.sy_train_targ,
                        inc_var_loss=False)[:, :i + 1],
                                   axis=-1) for i in range(self.num_plan_steps)
                ]
                self.reward_loss = [
                    tf.reduce_mean(self._compile_rew_dyn_losses(
                        self.sy_train_in, self.sy_train_targ)[0][:, :i + 1],
                                   axis=-1) for i in range(self.num_plan_steps)
                ]
                self.next_state_loss = [
                    tf.reduce_mean(self._compile_rew_dyn_losses(
                        self.sy_train_in, self.sy_train_targ)[1][:, :i + 1],
                                   axis=-1) for i in range(self.num_plan_steps)
                ]
                if self.reward_classification:
                    self.reward_acc = [
                        tf.reduce_mean(self._compile_rew_prediciton_accuracy(
                            self.sy_train_in,
                            self.sy_train_targ)[0][:, :i + 1],
                                       axis=-1)
                        for i in range(self.num_plan_steps)
                    ]

                self.train_op = [
                    self.optimizer.minimize(tf.reduce_mean(train_loss[:i + 1]),
                                            var_list=self.optvars)
                    for i in range(self.num_plan_steps)
                ]
    ...
    def load_params(self):
        with self.sess.as_default():
            params_dict = loadmat(
                os.path.join(self.model_dir, "%s_0.mat" % self.name))
    ...
    def _save_state(self, idx):
        self._state[idx] = [
            layer.get_model_vars(idx, self.sess) for layer in self.layers
        ]
        if self.separate_mean_var:
            self._state[idx].extend([
                layer.get_model_vars(idx, self.sess)
    ...
    def _set_state(self):
        ...
        if self.separate_mean_var:
            ...
            for layer in range(num_layers):
                ...
                params = {
                    key: np.stack([
                        self._state[net][len(self.layers) + layer][key]
                        for net in range(self.num_nets)
                    ])
                    for key in keys
                }
    ...
    def _save_best(self, epoch, holdout_losses, max_epochs=None):
        updated = False
        if not self.multi_step_prediction or (
                max_epochs is not None
                and epoch == max_epochs - 1) or epoch >= 100:  #120:#180:#120:
    ...
    def train(self,
        ...
        targets, holdout_targets = targets[permutation[num_holdout:]], targets[
            permutation[:num_holdout]]
        if self.multi_step_prediction:
            holdout_inputs = np.tile(holdout_inputs[None],
                                     [self.num_nets, 1, 1, 1])
            holdout_targets = np.tile(holdout_targets[None],
                                      [self.num_nets, 1, 1, 1])
        else:
            holdout_inputs = np.tile(holdout_inputs[None],
                                     [self.num_nets, 1, 1])
        ...
        for epoch in epoch_iter:
            if self.multi_step_prediction:  # False
                train_op = self.train_op[min(epoch // 50,
                                             self.num_plan_steps - 1)]
                mse_loss = self.mse_loss[min(epoch // 50,
                                             self.num_plan_steps - 1)]
                reward_loss = self.reward_loss[min(epoch // 50,
                                                   self.num_plan_steps - 1)]
                next_state_loss = self.next_state_loss[min(
                    epoch // 50, self.num_plan_steps - 1)]
                if self.reward_classification:
                    reward_acc = self.reward_acc[min(epoch // 50,
                                                     self.num_plan_steps - 1)]
            else:
                train_op = self.train_op
                mse_loss = self.mse_loss
                reward_loss = self.reward_loss
                next_state_loss = self.next_state_loss
                if self.reward_classification:
            ...
            if not hide_progress:
                if not self.reward_classification:
                    loss_tensors = [mse_loss, reward_loss, next_state_loss]
                else:
                    loss_tensors = [
                        mse_loss, reward_loss, next_state_loss, reward_acc
                    ]
                if holdout_ratio < 1e-12:  # False
                    losses = self.sess.run(loss_tensors,
                                           feed_dict={
                                               self.sy_train_in:
                    ...
                    named_losses = [['M{}'.format(i), losses[0][i]]
                                    for i in range(len(losses[0]))]
                    named_losses += [['R{}'.format(i), losses[1][i]]
                                     for i in range(len(losses[1]))]
                    named_losses += [['S{}'.format(i), losses[2][i]]
                                     for i in range(len(losses[2]))]
                    if self.reward_classification:
                        named_losses += [['Acc{}'.format(i), losses[3][i]]
                    ...
                    holdout_losses = self.sess.run(loss_tensors,
                                                   feed_dict={
                                                       self.sy_train_in:
                                                       holdout_inputs,
                                                       self.sy_train_targ:
                    ...
                    named_losses = [['M{}'.format(i), losses[0][i]]
                                    for i in range(len(losses[0]))]  # mse_loss
                    named_losses += [['R{}'.format(i), losses[1][i]]
                                     for i in range(len(losses[1]))
                                     ]  # reward_loss
                    named_losses += [['S{}'.format(i), losses[2][i]]
                    ...
                    named_holdout_losses = [[
                        'V{}'.format(i), holdout_losses[0][i]
                    ] for i in range(len(holdout_losses[0]))]
                    named_holdout_losses += [[
                        'VR{}'.format(i), holdout_losses[1][i]
                    ] for i in range(len(holdout_losses[1]))]
                    named_holdout_losses += [[
                        'VS{}'.format(i), holdout_losses[2][i]
                    ] for i in range(len(holdout_losses[2]))]
                    if self.reward_classification:
                        named_holdout_losses += [[
                            'VAcc{}'.format(i), holdout_losses[3][i]
                        ] for i in range(len(holdout_losses[3]))]
                    named_losses = named_losses + named_holdout_losses + [[
                        'T', time.time() - t0
                    ]]
                    progress.set_description(named_losses)

                    break_train = self._save_best(epoch,
                                                  holdout_losses[0],
                                                  max_epochs=max_epochs)

            progress.update()
            t = time.time() - t0
            if not no_early_stop and (break_train or
                                      (max_grad_updates
        ...
        train_losses = self.sess.run(loss_tensors,
                                     feed_dict={
                                         self.sy_train_in:
                                         inputs[idxs[:, :max_logging]],
                                         self.sy_train_targ:
        ...
        self._end_train(holdout_losses[0])
        if timer: timer.stamp('bnn_end')

        val_loss = (np.sort(holdout_losses[0])[:self.num_elites]).mean()
        val_r_loss = (np.sort(holdout_losses[1])[:self.num_elites]).mean()
        val_s_loss = (np.sort(holdout_losses[2])[:self.num_elites]).mean()
        train_s_loss = (np.sort(train_losses[2])[:self.num_elites]).mean()
        if self.reward_classification:
            val_r_acc = (np.sort(holdout_losses[3])[:self.num_elites]).mean()
            model_metrics = {
                'val_loss': val_loss,
                'val_r_loss': val_r_loss,
                'val_s_loss': val_s_loss,
                'val_r_acc': val_r_acc,
                'val_train_s_loss_diff': val_s_loss - train_s_loss
            }
        else:
            model_metrics = {
                'val_loss': val_loss,
        ...
        print('[ BNN ] Holdout', np.sort(holdout_losses[0]), model_metrics)
    ...
    def create_prediction_tensors(self,
        ...
        factored_mean, factored_variance = self._compile_outputs(inputs,
                                                                 one_step=True)
        if self.reward_classification:
            reward_mean = factored_mean[..., 0]
            reward_mean = tf.round(tf.nn.sigmoid(reward_mean))
            factored_mean = tf.concat(
                [tf.expand_dims(reward_mean, axis=-1), factored_mean[..., 1:]],
                axis=-1)
    ...
    def save(self, savedir, timestep):
        ...
        if not self.separate_mean_var:
            with open(os.path.join(model_dir, '{}_{}.nns'.format(self.name, timestep)), "w") as f:
                ...
                last_layer_copy.set_output_dim(
                    last_layer_copy.get_output_dim() // 2)
            ...
            with open(
                    os.path.join(model_dir,
                                 '{}_{}_var.nns'.format(self.name, timestep)),
                    "w+") as f:
                for layer in self.var_layers:
                    f.write("%s\n" % repr(layer))
    ...
    def _load_structure(self):
        ...
        if self.separate_mean_var:
            with open(os.path.join(self.model_dir, "%s_var_0.nns" % self.name),
                      "r") as f:
                for line in f:
                    kwargs = {
                        key: val
    ...
    def _compile_outputs(self, inputs, ret_log_var=False, one_step=False):
        ...
        Returns: (tf.Tensors) The mean and variance/log variance predictions at inputs for each network
            in the ensemble.
        """
        if not self.multi_step_prediction or one_step:  # True
            if not self.separate_mean_var:  # True
            ...
            for i in range(self.num_plan_steps):
                ...
                # obs = mean[:, :, 1:] + tf.random.normal(tf.shape(mean[:, :, 1:]))*tf.exp(logvar[:, :, 1:]) + obs
                obs = mean[:, :, 1:] + obs
                inputs = tf.concat([
                    obs, actions[:, :,
                                 min(i + 1, self.num_plan_steps - 1), :]
                ],
                                   axis=-1)
            mean = tf.transpose(tf.stack(mean_list), [1, 2, 0, 3])
            logvar = tf.transpose(tf.stack(logvar_list), [1, 2, 0, 3])
        if ret_log_var:
            return mean, logvar
        else:
    ...
    def _compile_losses(self, inputs, targets, inc_var_loss=True):
        ...
        if not self.reward_classification:  # True
            if inc_var_loss:
                ...
                total_losses = tf.reduce_mean(tf.reduce_mean(
                    tf.square(mean - targets), axis=-1),
                                              axis=-1)
        # False
        else:
            if inc_var_loss:
                mse_losses = tf.reduce_mean(tf.reduce_mean(
                    tf.square(mean[..., 1:] - targets[..., 1:]) *
                    inv_var[..., 1:],
                    axis=-1),
                                            axis=-1)
                var_losses = tf.reduce_mean(tf.reduce_mean(log_var[..., 1:],
                                                           axis=-1),
                                            axis=-1)
                next_state_loss = mse_losses + var_losses
            else:
                next_state_loss = tf.reduce_mean(tf.reduce_mean(
                    tf.square(mean[..., 1:] - targets[..., 1:]), axis=-1),
                                                 axis=-1)
            reward_loss = tf.reduce_mean(
                tf.nn.sigmoid_cross_entropy_with_logits(labels=targets[..., 0],
                                                        logits=mean[..., 0]),
                axis=-1)
            total_losses = reward_loss + next_state_loss

        return total_losses

    def _compile_rew_dyn_losses(self, inputs, targets):
        """Helper method for compiling the loss functions for reward and next state predictions.
        ...
        Returns: (tf.Tensor) Two tensors representing the losses for reward and next state.
        """
        mean, log_var = self._compile_outputs(inputs, ret_log_var=True)
        inv_var = tf.exp(-log_var)

        if self.reward_classification:
            reward_losses = tf.reduce_mean(
                tf.nn.sigmoid_cross_entropy_with_logits(labels=targets[..., 0],
                                                        logits=mean[..., 0]),
                axis=-1)
        else:
            reward_losses = tf.reduce_mean(tf.square(mean[:, :, 0] -
                                                     targets[:, :, 0]),
                                           axis=-1)
        mse_losses = tf.reduce_mean(tf.reduce_mean(
            tf.square(mean[..., 1:] - targets[..., 1:]) * inv_var[..., 1:],
            axis=-1),
                                    axis=-1)
        var_losses = tf.reduce_mean(tf.reduce_mean(log_var[..., 1:], axis=-1),
                                    axis=-1)
        next_state_losses = mse_losses + var_losses

        return reward_losses, next_state_losses

    def _compile_rew_prediciton_accuracy(self, inputs, targets):
        mean, log_var = self._compile_outputs(inputs, ret_log_var=True)
        assert self.reward_classification

        predicted = tf.nn.sigmoid(mean[..., 0])
        correct_pred = tf.equal(tf.round(predicted), targets[..., 0])
