...
class GaussianPolicy(LatentSpacePolicy):
    def __init__(self,
        ...
        def pis_fn(inputs):
            ...
            pis = distribution.prob(actions)[:, None]
            return pis
        ...
        pis = tf.keras.layers.Lambda(
            pis_fn)([shift, log_scale_diag, actions])

        pis_for_action_input = tf.keras.layers.Lambda(
            pis_fn)([shift, log_scale_diag, self.actions_input])

        self.pis_model = tf.keras.Model(
            (*self.condition_inputs, self.actions_input),
            pis_for_action_input)
    ...
    def pis(self, conditions, actions):
        assert not self._deterministic, self._deterministic
        return self.pis_model([*conditions, actions])
