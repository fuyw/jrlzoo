...
class SimpleSampler(BaseSampler):
    ...
    def sample(self):
        ...
        if hasattr(self.env, "oracle_env"):
            if isinstance(self._current_observation, dict):
                self._current_observation = self.env.convert_to_active_observation(
                    self._current_observation)
            next_observation, reward, terminal, info = self.env.step(self._current_observation, action)
            reward, terminal = reward[0], terminal[0]
        else:        
