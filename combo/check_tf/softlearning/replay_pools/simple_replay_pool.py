...
class SimpleReplayPool(FlexibleReplayPool):
    ...
    #     observation_keys = tuple(self._observation_space.spaces.keys())
    #     observations = np.concatenate([
    #         all_samples['observations.{}'.format(key)]
    #         for key in observation_keys
    #     ], axis=-1)

    #     next_observations = np.concatenate([
    #         all_samples['next_observations.{}'.format(key)]
    #         for key in observation_keys
    #     ], axis=-1)

    #     all_samples['observations'] = observations
    #     all_samples['next_observations'] = next_observations
    #     return all_samples
